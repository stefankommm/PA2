...
public:
    ...
    CWidget(const CWidget &copyFrom) = default;
...
public:
    CLabel(int id,
           const CRect &relPos,
           string label) : CWidget(id, relPos), m_label(move(label)) {};
...
public:
    CInput(int id, const CRect relPos, string value) : CWidget(id, relPos), m_value(move(value)) {};
...
public:
    ...
    void printOnly(ostream &os) const override {
        os << "[" << c_id << "] ComboBox " << c_absPos << "\n";
...
public:
    CPanel(int id, const CRect &relPos) : CWidget(id, relPos) {};

    CPanel(const CPanel &toCopy) : CWidget(toCopy.c_id, toCopy.c_relPos) {
    ...
    shared_ptr<CWidget> getCopy() const override {
    ...
    CWidget *search(const int id) const {
        ...
        for (const auto &child: c_children) {
            auto *panel = dynamic_cast<CPanel *>(child.get());
            if (panel) {
    ...
    friend ostream &operator<<(ostream &os, const CPanel &x) {
        ...
        while (it != x.c_children.end()) {
            auto &elem = *it;
            bool is_last = (it == it_last);
            auto is_ccombo = (dynamic_cast<CComboBox *>(it->get()) != nullptr);
            auto is_cpanel = (dynamic_cast<CPanel *>(it->get()) != nullptr);
...
public:
    ...
    CWindow(int id, string title, const CRect &absPos) : c_id(id), c_title(move(title)), c_absPos(absPos) {};
    ...
    CWidget *search(const int id) {
        ...
        for (const auto &child: c_children) {
            auto *panel = dynamic_cast<CPanel *>(child.second.get());
            if (panel) {
                CWidget *result = panel->search(id);
                if (result) {
                    return result;
...
int main(void) {
    ...
    assert(toString(abc) ==
           ...
           "|  +- [13] Panel (118,254.8,384,235.2)\n"
           "|  |  +- [1] Button \"Ok\" (156.4,442.96,115.2,23.52)\n"
           "|  |  +- [14] Panel (156.4,325.36,307.2,164.64)\n"
           "|  |  |  +- [15] Panel (187.12,374.752,245.76,115.248)\n"
           "|  |  |  +- [1] Button \"Ok\" (187.12,457.072,92.16,16.464)\n"
           "|  |  +- [1] Button \"Ok\" (156.4,442.96,115.2,23.52)\n"
           "|  +- [1] Button \"Ok\" (118,422.8,144,33.6)\n"
           "+- [13] Panel (70,154,480,336)\n");
